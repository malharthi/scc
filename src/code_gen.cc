...
void CodeGenerator::GenerateCode() {
  ...
  for (it = intermediate_code->begin(); it != intermediate_code->end(); it++) {
    ...
    case PRINT_INT_OP:
      EmitInstruction("mov", "eax", interm_instr->operand1()->GetAsmOperand(*this));
    ...
    case RETURN_OP:
      ...
      // The code for clearing the stack is generated by the parser before generating the
      // return statement. So before any return instruction there should be an
      // INC_STACK_PTR instruction that generates: add esp, stack_size
